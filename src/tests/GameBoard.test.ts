...
it("default game board", () => {
  ...
  expect(GameBoard.placeNewShip(ship1)).toBe(true);
});
...
it("ship placements", () => {
  ...
  expect(GameBoard.placeNewShip(ship3)).toBe(false);

  let ship4 = ShipFactory(10, [25, 25], Orientation.SIDEWAYS);
  expect(GameBoard.placeNewShip(ship4)).toBe(true);
});
